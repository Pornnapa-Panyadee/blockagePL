...
class DataForExpertController extends Controller
{
    public function reportExpert($blk_id=0){
        ...
        if(empty($photo_Blockage[0]->thumbnail_name)==true){
            $photo_has=0;
        }else if(empty($photo_Blockage[1]->thumbnail_name)==true){
            $photo_has=1;
        }else{
            $photo_has=2;
        }
    ...
    public function expertPDFSurvey($blk_id=0)
    {
        ...
        $current_end = json_decode($current_end);
        // dd($expert);
        $pdf = PDF::loadView('expert.reportpdf_survey' ,compact('current_brigde','expert','data','nut','hum','damageData','damage_type','pastData','current_start','current_narrow_new','current_end','problem','photo_Blockage','photo_Land','photo_Riverbefore','photo_Riverprob','photo_Riverafter','photo_Probsketch','solution_id','project_id'));
       ...
       // dd($request->photo_type_land);
        ...
        if ($request->hasFile('photo_type_land')) {
            ...
            foreach($images as $key => $image) {
                ...
                $blockageId=$blk_id;
                $filename = $blockageId.'.'.$image->getClientOriginalExtension();
                ...
                if (($org_img && $thm_img) == true) {
                    // Image::make($image)->fit(3308, 4675, function ($constraint) {
                    //         $constraint->upsize();
                    //     })->save($org_path);
                    Image::make($image)->fit(1654, 2338, function ($constraint) {
                        $constraint->upsize();
                    })->save($thm_path);
                }

                $ph = Expert::where('blk_id', $blk_id)->update([  'exp_pixmap'=>$thm_path]);
        ...
        if(Auth::guest()){
            ...
            if(Auth::user()->status_work=="surveyor1"){
                if(isset($data)){
                    ...
                    return view('pages.test',compact('data','districtData','username','lastname','position','department'));
            }            
        }
    ...
    public function getDataforHome1(User $user, Request $request)
    {
        ...
        if(Auth::guest()){
            return view('pages.box',compact('data','districtData'));
        }else{
            return view('pages.box',compact('data','districtData'));
        }
